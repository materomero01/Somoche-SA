#!/bin/bash
#
# Cliente del WSAA para ws_sr_constancia_inscripcion (Padrón A5) en Linux.
...
if echo "${RESPONSE}" | grep -q "<soapenv:Fault>"; then
    ...
    echo "${RESPONSE}" > "${SEQ_NR}-loginTicketResponse_${SERVICIO_ID}-ERROR.xml"
    echo "${RESPONSE}"
    exit 1
fi

# Extraer el contenido del loginCmsReturn usando sed
